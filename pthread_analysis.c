...
int checkEnv (int sto_no, struct storage_info *p)
{
	...
	if (0 == tem_alarm_status[sto_no])
	{
		if (p->temperature > p->temperatureMAX)
		 {
			sendMsgQueue (MSG_LED, MSG_LED_TEM_ON);     //fs6818上的灯
...
void *pthread_analysis (void *arg)
{
	...
	while (1)
	{
		...
		pthread_mutex_unlock (&mutex_analysis);	//多余解锁
...
//		printf ("wake pthread_analysis wake up\n");
		while (1)
		{
			...
			free (node);
			pthread_mutex_unlock (&mutex_linklist);
//			printf("buf.msg_type = %c\n",buf.msg_type);
			if ('e' == buf.msg_type)
			{
				getEnvPackage (&buf);
//				printf("pthread_analysis getEnvPackage()");
			}
